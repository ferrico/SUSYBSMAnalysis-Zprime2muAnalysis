    ...
    hardInteraction(fill_gen_info ? new HardInteraction(cfg.getParameter<edm::ParameterSet>("hardInteraction")) : 0)
{
  ...
  LeptonPt_b = fs->make<TH1F>("LeptonPt_b", titlePrefix + "pT", 5000, 0, 5000);
  LeptonPt_bo = fs->make<TH1F>("LeptonPt_bo", titlePrefix + "pT", 5000, 0, 5000);
  LeptonPt_oe = fs->make<TH1F>("LeptonPt_eo", titlePrefix + "pT", 5000, 0, 5000);
  LeptonPt_e = fs->make<TH1F>("LeptonPt_e", titlePrefix + "pT", 5000, 0, 5000);
...
void Zprime2muHistosFromPAT::fillBasicLeptonHistos(const reco::CandidateBaseRef& lep) {
  ...
  if (lep->eta() <= 0.9)
  	LeptonPt_b->Fill(lep->pt(), _madgraphWeight*_kFactor);
  if (lep->eta() <= 1.2)
  	LeptonPt_bo->Fill(lep->pt(), _madgraphWeight*_kFactor);
  if (lep->eta() > 0.9)
  	LeptonPt_oe->Fill(lep->pt(), _madgraphWeight*_kFactor);
  if (lep->eta() > 1.2)
  	LeptonPt_e->Fill(lep->pt(), _madgraphWeight*_kFactor);
...
void Zprime2muHistosFromPAT::fillOfflineMuonHistos(const pat::Muon* mu) {
  ...
  if (track.isAvailable()) {
    ...
    NInvalidHits->Fill(hp.numberOfHits(reco::HitPattern::TRACK_HITS) - hp.numberOfValidHits(), _madgraphWeight*_kFactor);
...
// 			 	}
				...
				gM = gM - 400;
			   	_kFactor = 1.067 - 0.000112 * gM + 3.176E-8 * pow(gM,2) - 4.068E-12 * pow(gM,3);
			   	_kFactor_bb = 1.036 - 0.0001441 * gM + 5.058e-8 * pow(gM,2) - 7.581e-12 * pow(gM,3);
	 		   	_kFactor_be = 1.052 - 0.0001471 * gM + 5.903e-8 * pow(gM,2) - 9.037e-12 * pow(gM,3);
