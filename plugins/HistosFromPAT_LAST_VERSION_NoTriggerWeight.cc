...
//   std::cout<<"                                       i tau hanno: +/- 15: "<<lep->pdgId()<<std::endl;
//   if(abs(lep->pdgId()) == 15)
...
void Zprime2muHistosFromPAT::fillDileptonHistos(const pat::CompositeCandidate& dil, const edm::Event& event) {
  ...
  DileptonMass_bb->Fill(dil.mass());
    
  if (dil.daughter(0)->eta()<-1.2 || dil.daughter(1)->eta()<-1.2)   
  DileptonMass_ne->Fill(dil.mass());

  if ((dil.daughter(0)->eta()>1.2 && dil.daughter(1)->eta()>=-1.2) || (dil.daughter(0)->eta()>=-1.2 && dil.daughter(1)->eta()>1.2))
  DileptonMass_pe->Fill(dil.mass());
  ...
  if (dil.hasUserFloat("vertexM") && dil.hasUserFloat("vertexMError")) {
    ...
    if (dil.daughter(0)->eta()<=1.2 && dil.daughter(1)->eta()<=1.2 && dil.daughter(0)->eta()>=-1.2 && dil.daughter(1)->eta()>=-1.2)
    DimuonMassVertexConstrained_bb->Fill(vertex_mass);
    
    if (dil.daughter(0)->eta()<-1.2 || dil.daughter(1)->eta()<-1.2)   
	DimuonMassVertexConstrained_ne->Fill(vertex_mass);

    if ((dil.daughter(0)->eta()>1.2 && dil.daughter(1)->eta()>=-1.2) || (dil.daughter(0)->eta()>=-1.2 && dil.daughter(1)->eta()>1.2))
	DimuonMassVertexConstrained_pe->Fill(vertex_mass);
