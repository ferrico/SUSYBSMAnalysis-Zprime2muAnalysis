    ...
    hardInteraction(cfg.getParameter<edm::ParameterSet>("hardInteraction"))    
{
  ...
  for (size_t i = 0; i < triggerDecision.l1_paths().size(); ++i) {
    ...
    l1_path_effs_bb.push_back(make_eff_pair(TString::Format("L1Path_bb_%u_%s", unsigned(i), triggerDecision.l1_paths().at(i).c_str()), TString::Format("#varepsilon(%s) vs. mass", triggerDecision.l1_paths().at(i).c_str())));
  ...
  total_trig_eff_e = make_eff_pair("TotalTrigEff_e", TString::Format("#varepsilon((%s) && (%s)) vs. mass", join(triggerDecision.l1_paths(), std::string(" || ")).c_str(), join(triggerDecision.hlt_paths(), std::string(" || ")).c_str()));
}
...
//   }
  if (!hardInteraction.IsValidForRes()) {
    edm::LogWarning("EfficiencyFromMC") << "!hardInteraction.IsValidForRes()";
    return;
  }
...
//   const double m_denom = use_resonance_mass_denom ? hardInteraction.resonance->mass() : hardInteraction.dilepton().mass();
  const double m = hardInteraction.resonance->mass();
  const double m_denom = hardInteraction.resonance->mass();
...
//   const double aeta_plus  = fabs(hardInteraction.lepPlus ->eta());
  const double aeta_minus = fabs(hardInteraction.lepMinusNoIB->eta());
  const double aeta_plus  = fabs(hardInteraction.lepPlusNoIB ->eta());
...
//       hardInteraction.lepPlus ->pt() > acceptance_min_pt)
      hardInteraction.lepMinusNoIB->pt() > acceptance_min_pt &&
      hardInteraction.lepPlusNoIB ->pt() > acceptance_min_pt)
  {
  ...
  if (check_l1) {
    for (size_t i = 0; i < triggerDecision.l1_paths().size(); ++i) {
      ...
      if (aeta_minus <= 1.2 && aeta_plus <= 1.2) {
        l1_path_effs_bb[i].first->Fill(m);
      } // BB

      else {
        l1_path_effs_e[i].first->Fill(m);
      } // BE + EE
...
//         obj.unpackFilterLabels(event, *triggerBits); // 92X
  ...
  if (l1_or) {
    l1_or_eff.first->Fill(m);

    if (aeta_minus <= 1.2 && aeta_plus <= 1.2) {
      l1_or_eff_bb.first->Fill(m);
    } // BB

    else { 
      l1_or_eff_e.first->Fill(m);
    } // BE + EE
  }
  ...
  if (l1_or && hlt_or)
  {
    ...
    if (aeta_minus <= 1.2 && aeta_plus <= 1.2) {
      total_trig_eff_bb.first->Fill(m);
      recowrtacctrig_bb.second->Fill(m);
    } // BB

    else {
      total_trig_eff_e.first->Fill(m);
...
//         (reco::deltaR(*dau0, *hardInteraction.lepMinus) < dRmax || reco::deltaR(*dau1, *hardInteraction.lepMinus) < dRmax)) 
    if ((reco::deltaR(*dau0, *hardInteraction.lepPlusNoIB)  < dRmax || reco::deltaR(*dau1, *hardInteraction.lepPlusNoIB)  < dRmax) &&
        (reco::deltaR(*dau0, *hardInteraction.lepMinusNoIB) < dRmax || reco::deltaR(*dau1, *hardInteraction.lepMinusNoIB) < dRmax)) 
    {
