...
GeneralizedEndpoint::GeneralizedEndpoint(){
  ...
  //[-1.2, 0.]
  _Correction[2][0] =  0.0;      _CorrectionError[2][0] = 0.025;//.03;
  _Correction[2][1] =  0.0;      _CorrectionError[2][1] = 0.025;//.03;
  _Correction[2][2] =  0.0;      _CorrectionError[2][2] = 0.025;//.03;
  //[-0., 1.2]
  _Correction[3][0] =  0.0;      _CorrectionError[3][0] = 0.025;//.03;
  _Correction[3][1] =  0.0;      _CorrectionError[3][1] = 0.025;//.03;
  _Correction[3][2] =  0.0;      _CorrectionError[3][2] = 0.025;//.03;
  //[1.2, 2.1]
...
//   if (verbose ==1) printf("eta bin %i, phi bin %i Correction %f +- %f pt %f\n", kEtaBin, kPhiBin, KappaBias, KappaBiasError,MuonPt);
  if (verbose ==1) printf("--- eta bin %i --- %f ----, phi bin %i Correction %f +- %f pt %f\n", kEtaBin,MuonEta, kPhiBin, KappaBias, KappaBiasError,MuonPt);

  MuonPt = MuonPt/1000.; //GeV to TeV.
  MuonPt = fabs(MuonCharge)*fabs(MuonPt); //Signed Pt.
