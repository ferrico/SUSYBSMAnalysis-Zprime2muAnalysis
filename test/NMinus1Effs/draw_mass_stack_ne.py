...
psn = 'plots/categories/ne/mass_stack'
        ...
        if self.fn is not None:
            ...
            for nminus1 in nminus1s + ['NoNo']:
                if nminus1=='NoVtxProb':
                    ...
                    self.histos[nminus1] = f.Get(nminus1).Get('DimuonMassVertexConstrained_ne').Clone()#DileptonMass
                else:
                    self.histos[nminus1] = f.Get(nminus1).Get('DimuonMassVertexConstrained_ne').Clone()#DileptonMass
        ...
        for nminus1 in nminus1s + ['NoNo']:
            ...
            for sample in samples:
                ...
                if nminus1 == 'NoVtxProb':
                    ...
                    h = f.Get(nminus1).Get('DimuonMassVertexConstrained_ne').Clone()
                else:
                    h = f.Get(nminus1).Get('DimuonMassVertexConstrained_ne').Clone()
...
#                elif mc.name == 'WWinclusive':
    ...
    t = ROOT.TPaveLabel(0.50, 0.425, 0.90, 0.525, 'category: E-', 'brNDC')
    t.SetTextFont(42)
    t.SetTextSize(0.5)
    t.SetBorderSize(0)
    t.SetFillColor(0)
