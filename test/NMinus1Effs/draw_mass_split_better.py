...
def draw_mass_test(tag, printStats, lumi, do_tight=False):
    ...
    psn =  'plots/' + period  + categoria_1 + cartella + '/mass_split/'
    if tag:
        psn =  'plots/' + period  + categoria_1 + cartella + '/mass_split/'
...
#                    print '%s ----- %s ---- %s' % (denTot, denInt, mc.partial_weight)
            ...
            eff.GetYaxis().SetTitle('n-1 efficiency')
            draw = '2'
            eff.SetLineColor(ROOT.kCyan+2)
            eff.SetFillColor(ROOT.kCyan+2)
            eff.SetFillStyle(1001)
...
#                         print "Sample: %s ---- Num = %d --- Den = %d" %(mc.name, numInt, denInt)
                        if numInt<=denInt and denInt>0:
                            p_hat_mc = float(numInt)/denInt
                            err2 = p_hat_mc*(1-p_hat_mc)/denInt
                        elif numInt>denInt:
...
if __name__=='__main__':
    ...
    tag = args.tag
    if not os.path.exists( 'plots/' + period  + categoria_1 + cartella + '/mass/'):
        raise ValueError( 'plots/' + period  + categoria_1 + cartella + '/mass_split/')
