...
// 			if(j > 30) std::cout<<genWeight;
		...
		if(
			...
			vertex_chi2 < 20
				...
				if(j >= 30 && j <=38) weight[j] *= kFactor;
				if(j >= 30 && j <=38) weight_BB[j] *= kFactor_BB;
				if(j >= 30 && j <=38) weight_BE[j] *= kFactor_BE;

					dimuon_MC_clear->Fill(dil_mass,  weight[j]); 
					if(fabs(lep_eta[0]) < 1.2 && fabs(lep_eta[1]) < 1.2)
						dimuon_BB_MC_clear->Fill(dil_mass,  weight_BB[j]);
					else
						dimuon_BE_MC_clear->Fill(dil_mass,  weight_BE[j]);
					...
					for(int h = 0; h < 2; h++){ // for on two muons in the event									    						
						...
						if(fabs(lep_eta[h]) < 1.2)
							pt_MC_BB_clear->Fill(lep_pt[h],  weight_BB[j]);
						else
							pt_MC_BE_clear->Fill(lep_pt[h],  weight_BE[j]);
...
// 		TH1F* dimuon_MC_clear = (TH1F*)gDirectory->Get("DimuonMassVtxConstrainedLog");
// 		TH1F* dimuon_BB_MC_clear = (TH1F*)gDirectory->Get("DimuonMassVtxConstrainedLog_bb");
// 		TH1F* dimuon_BE_MC_clear = (TH1F*)gDirectory->Get("DimuonMassVtxConstrainedLog_be");
// 		TH1F* pt_MC_clear = (TH1F*)gDirectory->Get("LeptonPt");
// 		TH1F* eta_MC_clear = (TH1F*)gDirectory->Get("LeptonEta");
// 		TH1F* phi_MC_clear = (TH1F*)gDirectory->Get("LeptonPhi");
...
// 
// 		dimuon_MC_clear->Scale(weight[j]);
// 		dimuon_BB_MC_clear->Scale(weight[j]*Z_peak_BB/Z_peak);
// 		dimuon_BE_MC_clear->Scale(weight[j]*Z_peak_BE/Z_peak);
// 		pt_MC_clear->Scale(weight[j]);
// 		pt_MC_clear->Rebin(50);
// // 		eta_MC_clear->Scale(weight[j]);
// // 		phi_MC_clear->Scale(weight[j]);
...
// 				if(j >= 6 && j <=14) weight_2017[j] *= kFactor;
// 				if(j >= 6 && j <=14) weight_2017_BB[j] *= kFactor_BB;
// 				if(j >= 6 && j <=14) weight_2017_BE[j] *= kFactor_BE;
...
// 			weight_2017[j] /= kFactor;
// 			weight_2017_BB[j] /= kFactor_BB;
// 			weight_2017_BE[j] /= kFactor_BE;
