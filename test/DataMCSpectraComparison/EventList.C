...
// 		weight[i] = 1;
	...
	FILE *Total_List = fopen("Total_List.txt", "w");
	FILE *BB_List = fopen("BB_List.txt", "w");
	FILE *BE_List = fopen("BE_List.txt", "w");
                        	
     TChain *treeDATA = new TChain("SimpleNtupler/t");

     treeDATA->Add("/eos/user/f/ferrico/LXPLUS/ROOT_FILE_2017/DATA_WITH_OR/ana_datamc_data.root");        
...
// 				std::cout<<event<<"\t"<<dil_mass<<"\t"<<lep_pt[0] + lep_pt[1]<<std::endl;
...
// 				std::cout<<p<<"\t"<<event<<"\t"<<dil_mass<<"\t"<<lep_pt[0] + lep_pt[1]<<std::endl;
				...
				if(dil_mass > 150){
					...
					fprintf(Total_List, "%d\t%f\t%f\n", event, dil_mass, lep_pt[0] + lep_pt[1]);
