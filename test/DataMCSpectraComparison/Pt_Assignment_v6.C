...
//   float gen_lep_pt[2];
  ...
  float met_pt;
	...
	int count_assignment_MC_till600[8][25];
	int count_assignment_DATA_till600[8];
	int count_assignment_MC_above600[8][25];
	...
	TH1D* h_count_assignment_MC_till600[8];
	TH1D* h_count_assignment_DATA_till600[8];
...
//     gStyle->SetPadRightMargin(0.05);
	...
	THStack* eta_GEN_till600_Stack = new THStack("#eta gen: pT_{gen} < 600 GeV", "#eta gen: pT_{gen} < 600 GeV");
	THStack* phi_GEN_till600_Stack = new THStack("#phi gen: pT_{gen} < 600 GeV", "#phi gen: pT_{gen} < 600 GeV");		
	THStack* eta_GEN_above600_Stack = new THStack("#eta gen: pT_{gen} > 600 GeV", "#eta gen: pT_{gen} > 600 GeV");
	THStack* phi_GEN_above600_Stack = new THStack("#phi gen: pT_{gen} > 600 GeV", "#phi gen: pT_{gen} > 600 GeV");		
...
// 	TH2F* std_vs_tuneP_DATA = new TH2F("Std vs TuneP: DATA", "Std vs TuneP: DATA", 1800, 200, 1800, 1800, 200, 2000);
     ...
     for(int j = 14; j <39; j++){
		...
		TH1F* eta_GEN_till600_clear = new TH1F("#eta gen: pT_{gen} < 600 GeV", "#eta gen: pT_{gen} < 600 GeV", binnum_eta, ETA_BINS);
		TH1F* eta_GEN_above600_clear = new TH1F("#eta gen: pT_{gen} > 600 GeV", "#eta gen: pT_{gen} > 600 GeV", binnum_eta, ETA_BINS);
		TH1F* phi_GEN_till600_clear = new TH1F("#phi gen: pT_{gen} < 600 GeV", "#phi gen: pT_{gen} < 600 GeV", binnum_phi, PHI_BINS);
		TH1F* phi_GEN_above600_clear = new TH1F("#phi gen: pT_{gen} > 600 GeV", "#phi gen: pT_{gen} > 600 GeV", binnum_phi, PHI_BINS);
...
// 	     				numberOfValidMuonHits[h][5] = lep_TuneP_numberOfValidMuonHits[h];
	     				...
	     				if (
    	 					(lep_numberOfMatchedStations[h] > 1 || (lep_numberOfMatchedStations[h] == 1 && !(lep_stationMask[h] == 1 || lep_stationMask[h] == 16)) || (lep_numberOfMatchedStations[h] == 1 &&(lep_stationMask[h] == 1 || lep_stationMask[h] == 16) && lep_numberOfMatchedRPCLayers[h] > 2))
 	     					...
 	     					if(gen_lep_pt[h] < 600){
		     					eta_GEN_till600_clear->Fill(gen_lep_eta[h],  weight[j]);
		     					phi_GEN_till600_clear->Fill(gen_lep_phi[h],  weight[j]);
		     				}
		     				else{
		     					eta_GEN_above600_clear->Fill(gen_lep_eta[h],  weight[j]);
...
//      								if(i != 7) count_assignment_MC_till600[i][j-14]++;
		     						...
		     						if(lepton_pt[h][i] < 600){
	     								count_assignment_MC_till600[i][j-14]++;
	     								...
	     								if(i != 7) count_assignment_MC_above600[i][j-14]++;
...
//     	 						count_assignment_MC_till600[7][j-14]++;
...
//      						std::cout<<count_assignment_MC_till600[0][0]<<"\t"<<count_assignment_MC_till600[1][0]<<"\t"<<count_assignment_MC_till600[2][0]
//      						<<"\t"<<count_assignment_MC_till600[3][0]<<"\t"<<count_assignment_MC_till600[4][0]<<"\t"<<count_assignment_MC_till600[5][0]
//      						<<"\t"<<count_assignment_MC_till600[6][0]<<"\t"<<count_assignment_MC_till600[7][0]
//      						<<"\t\t\t"<<lepton_pt[h][0]<<"\t"<<lepton_pt[h][1]<<"\t"<<lepton_pt[h][2]<<"\t"<<lepton_pt[h][3]
//      						<<"\t"<<lepton_pt[h][4]<<"\t"<<lepton_pt[h][5]<<"\t"<<lepton_pt[h][6]<<"\t"<<lepton_pt[h][7]<<std::endl;

//      						for(int i =0; i < 8; i++)
//      							std::cout<<reconstruction[i].Data()<<"\t"<<count_assignment_MC_till600[i][j-30]<<std::endl;
//      						std::cout<<"-"<<std::endl;
     					}
...
// 				phi_MC_clear[i]->SetMarkerColor(kGreen+3);
		 ...
		 if(j > 30){
			...
			eta_GEN_till600_clear->SetFillColor(3);
			eta_GEN_till600_clear->SetLineColor(3);
			phi_GEN_till600_clear->SetFillColor(3);
			phi_GEN_till600_clear->SetLineColor(3);
		 ...
		 if(j == 30){
			...
			eta_GEN_till600_clear->SetFillColor(kGreen+3);
			eta_GEN_till600_clear->SetLineColor(kGreen+3);
			phi_GEN_till600_clear->SetFillColor(kGreen+3);
			phi_GEN_till600_clear->SetLineColor(kGreen+3);
		 ...
		 if(j > 27 && j < 30){
			...
			eta_GEN_till600_clear->SetFillColor(kRed+3);
			eta_GEN_till600_clear->SetLineColor(kRed+3);
			phi_GEN_till600_clear->SetFillColor(kRed+3);
			phi_GEN_till600_clear->SetLineColor(kRed+3);
		 ...
		 if(j > 25 && j < 28){
			...
			eta_GEN_till600_clear->SetFillColor(2);
			eta_GEN_till600_clear->SetLineColor(2);
			phi_GEN_till600_clear->SetFillColor(2);
			phi_GEN_till600_clear->SetLineColor(2);
		 ...
		 if(j > 20 && j < 26){
			...
			eta_GEN_till600_clear->SetFillColor(kRed-10);
			eta_GEN_till600_clear->SetLineColor(kRed-10);
			phi_GEN_till600_clear->SetFillColor(kRed-10);
			phi_GEN_till600_clear->SetLineColor(kRed-10);
		 ...
		 if(j > 15 && j < 21){
			...
			eta_GEN_till600_clear->SetFillColor(4);
			eta_GEN_till600_clear->SetLineColor(4);
			phi_GEN_till600_clear->SetFillColor(4);
			phi_GEN_till600_clear->SetLineColor(4);
		 ...
		 if(j == 14 || j == 15){
			...
			eta_GEN_till600_clear->SetFillColor(kBlue+2);
			eta_GEN_till600_clear->SetLineColor(kBlue+2);
			phi_GEN_till600_clear->SetFillColor(kBlue+2);
			phi_GEN_till600_clear->SetLineColor(kBlue+2);
		 ...
		 eta_GEN_till600_Stack->Add(eta_GEN_till600_clear, "HIST");
		 phi_GEN_till600_Stack->Add(phi_GEN_till600_clear, "HIST");
...
//     	 							if(i == 2 && (lepton_pt[h][i] == lepton_pt[h][3] || lepton_pt[h][i] == lepton_pt[h][5])) continue;
									// Select DYT only if DYT = TuneP    	 						
// 									if(i == 4 && lepton_pt[h][i] == lepton_pt[h][5]) continue;
//      								if(i != 7) count_assignment_DATA_till600[i]++;
    	 					...
    	 					if(lep_pt[h] == lepton_pt[h][7]){ 
//     	 						count_assignment_DATA_till600[7]++;
...
//      						for(int i =0; i < 8; i++)
//      							std::cout<<reconstruction[i].Data()<<"\t"<<count_assignment_DATA_till600[i][j-30]<<std::endl;
//      						std::cout<<"-"<<std::endl;
     					}
     ...
     std::cout<<" Till 600 GeV "<<std::endl;
    ...
    for(int i = 0; i < 25; i++){ // samples
 		printf("%15s -->  %15d\t%15d\t%15d\t%15d\t%15d\t%15d\t%15d\t\t|%15d\n", samples[14+i].Data(), count_assignment_MC_till600[0][i], count_assignment_MC_till600[1][i], count_assignment_MC_till600[2][i], count_assignment_MC_till600[3][i], count_assignment_MC_till600[4][i], count_assignment_MC_till600[5][i], count_assignment_MC_till600[6][i], count_assignment_MC_till600[7][i]);
	   	for(int j = 1; j < 8; j++) // reconstruction
	    	tot[j] += count_assignment_MC_till600[j][i];
     }
     for(int i = 1; i < 7; i++){
     	TOT_MC+=tot[i];
     	TOT_DATA+=count_assignment_DATA_till600[i];
     }

 	 printf(" ----------------------------------------------------------------------------------------------------------------------------------- \n");
     printf("    MC         --->  %15d\t%15d\t%15d\t%15d\t%15d\t%15d\t%15d\t\t|%15d\n", tot[0], tot[1], tot[2], tot[3], tot[4],tot[5], tot[6], tot[7]);
 	 printf("  DATA         --->  %15d\t%15d\t%15d\t%15d\t%15d\t%15d\t%15d\t\t|%15d\n", count_assignment_DATA_till600[0], count_assignment_DATA_till600[1], count_assignment_DATA_till600[2], count_assignment_DATA_till600[3], count_assignment_DATA_till600[4], count_assignment_DATA_till600[5], count_assignment_DATA_till600[6], count_assignment_DATA_till600[7]);
 	 printf(" ----------------------------------------------------------------------------------------------------------------------------------- \n");
 	 printf("  MC TOTAL = %10d\nDATA TOTAL = %10d\n", TOT_MC, TOT_DATA); 	 
 	 printf(" ----------------------------------------------------------------------------------------------------------------------------------- \n");
     printf("   MC (%%)      ---> %15.3f\t%15.3f\t%15.3f\t%15.3f\t%15.3f\t%15.3f\t%15.3f\t\t|%15.3f\n", (float)tot[0]/TOT_MC, (float)tot[1]/TOT_MC, (float)tot[2]/TOT_MC,(float)tot[3]/TOT_MC, (float)tot[4]/TOT_MC, (float)tot[5]/TOT_MC, (float)tot[6]/TOT_MC, (float)tot[7]/TOT_MC);
 	 printf(" DATA (%%)      ---> %15.3f\t%15.3f\t%15.3f\t%15.3f\t%15.3f\t%15.3f\t%15.3f\t\t|%15.3f\n", (float)count_assignment_DATA_till600[0]/TOT_DATA, (float)count_assignment_DATA_till600[1]/TOT_DATA, (float)count_assignment_DATA_till600[2]/TOT_DATA, (float)count_assignment_DATA_till600[3]/TOT_DATA, (float)count_assignment_DATA_till600[4]/TOT_DATA, (float)count_assignment_DATA_till600[5]/TOT_DATA, (float)count_assignment_DATA_till600[6]/TOT_DATA, (float)count_assignment_DATA_till600[7]/TOT_DATA);
    ...
    for(int i = 0; i < 25; i++){ // samples
 		printf("%15s -->  %15d\t%15d\t%15d\t%15d\t%15d\t%15d\t%15d\t\t|%15d\n", samples[14+i].Data(), count_assignment_MC_above600[0][i], count_assignment_MC_above600[1][i], count_assignment_MC_above600[2][i], count_assignment_MC_above600[3][i], count_assignment_MC_above600[4][i], count_assignment_MC_above600[5][i], count_assignment_MC_above600[6][i], count_assignment_MC_above600[7][i]);
	   	for(int j = 1; j < 8; j++) // reconstruction
	    	tot[j] += count_assignment_MC_above600[j][i];
     }
     for(int i = 1; i < 7; i++){
     	TOT_MC+=tot[i];
 	 ...
 	 printf(" ----------------------------------------------------------------------------------------------------------------------------------- \n");
     printf("    MC         --->  %15d\t%15d\t%15d\t%15d\t%15d\t%15d\t%15d\t\t|%15d\n", tot[0], tot[1], tot[2], tot[3], tot[4],tot[5], tot[6], tot[7]);
 	 printf("  DATA         --->  %15d\t%15d\t%15d\t%15d\t%15d\t%15d\t%15d\t\t|%15d\n", count_assignment_DATA_above600[0], count_assignment_DATA_above600[1], count_assignment_DATA_above600[2], count_assignment_DATA_above600[3], count_assignment_DATA_above600[4], count_assignment_DATA_above600[5], count_assignment_DATA_above600[6], count_assignment_DATA_above600[7]);
 	 printf(" ----------------------------------------------------------------------------------------------------------------------------------- \n");
 	 printf("  MC TOTAL = %10d\nDATA TOTAL = %10d\n", TOT_MC, TOT_DATA); 	 
 	 printf(" ----------------------------------------------------------------------------------------------------------------------------------- \n");
     printf("   MC (%%)      ---> %15.3f\t%15.3f\t%15.3f\t%15.3f\t%15.3f\t%15.3f\t%15.3f\t\t|%15.3f\n", (float)tot[0]/TOT_MC, (float)tot[1]/TOT_MC, (float)tot[2]/TOT_MC,(float)tot[3]/TOT_MC, (float)tot[4]/TOT_MC, (float)tot[5]/TOT_MC, (float)tot[6]/TOT_MC, (float)tot[7]/TOT_MC);
 	 printf(" DATA (%%)      ---> %15.3f\t%15.3f\t%15.3f\t%15.3f\t%15.3f\t%15.3f\t%15.3f\t\t|%15.3f\n", (float)count_assignment_DATA_above600[0]/TOT_DATA, (float)count_assignment_DATA_above600[1]/TOT_DATA, (float)count_assignment_DATA_above600[2]/TOT_DATA, (float)count_assignment_DATA_above600[3]/TOT_DATA, (float)count_assignment_DATA_above600[4]/TOT_DATA, (float)count_assignment_DATA_above600[5]/TOT_DATA, (float)count_assignment_DATA_above600[6]/TOT_DATA, (float)count_assignment_DATA_above600[7]/TOT_DATA);
...
//  	bool save = true;
	...
	SalvaHisto("#eta gen distribution (pT_{gen} < 600 GeV): stack plot", eta_GEN_till600_Stack, eta_DATA_till600[7], dir_save + "Variuos_distribution.pdf[", 0,  0, "#eta");
	SalvaHisto("#eta gen distribution (pT_{gen} > 600 GeV): stack plot", eta_GEN_above600_Stack, eta_DATA_above600[7], dir_save + "Variuos_distribution.pdf[", 0,  0, "#eta");
	SalvaHisto("#phi gen distribution (pT_{gen} < 600 GeV): stack plot", phi_GEN_till600_Stack, phi_DATA_till600[7], dir_save + "Variuos_distribution.pdf[", 0,  0, "#phi");
	SalvaHisto("#phi gen distribution (pT_{gen} > 600 GeV): stack plot", phi_GEN_above600_Stack, phi_DATA_above600[7], dir_save + "Variuos_distribution.pdf[", 0,  0, "#phi");
