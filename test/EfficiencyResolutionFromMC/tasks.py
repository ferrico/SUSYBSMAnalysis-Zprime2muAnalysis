...
#    from SUSYBSMAnalysis.Zprime2muAnalysis.MCSamples import samples
#    for sample in samples:
#        do('crab -c crab/crab_effres_%(name)s -publish >& crab/publish_logs/publish.crab_effres_%(name)s &' % sample)
    ...
    for sample in samples:
        ana_dataset = None
        fn = 'crab/publish_logs/publish.crab_effres_%s' % sample.name
        # yay fragile parsing
        for line in open(fn):
    ...
    for lumi_mask in lumi_masks:
        print lumi_mask
#        dirs = glob.glob('crab/crab_ana_effres_%s_ExpressPhysicsRun2015B*' % lumi_mask)
        dirs = glob.glob('crab/crab_ana_effres_%s_SingleMuonRun2016*' % lumi_mask)
#        dirs = glob.glob('crab/crab_ana_effres_%s_ExpressPhysicsRun2015B-Express_251161_251252' % lumi_mask)
        files = []
        for d in dirs:
        ...
        os.mkdir(wdir)
        hadd(os.path.join(wdir, 'ana_effres_data.root'), files)
...
#             do('crab status -d %(dir)s ; crab report -d %(dir)s ' % locals())
        ...
        reduce(lambda x,y: x|y, (LumiList(j) for j in jsons)).writeJSON('%(wdir)s/ana_effres_data.forlumi.json' % locals())
        do('brilcalc lumi --normtag /afs/cern.ch/user/l/lumipro/public/normtag_file/normtag_DATACERT.json -u /pb  -i %(wdir)s/ana_effres_data.forlumi.json  > %(wdir)s/ana_effres_data.lumi -b "STABLE BEAMS" ' % locals())
        do('tail -5 %(wdir)s/ana_effres_data.lumi' % locals())
        print 'done with', lumi_mask, '\n'

        #do('lumiCalc2.py -i %(wdir)s/ana_effres_data.forlumi.json overview > %(wdir)s/ana_datamc_data.lumi' % locals())
