...
hists_const = []   
hists_dir_const = '../DataMCSpectraComparison/mc/DY_old_FUNCTION/'
for m,w in zip(masses, weights):
    fn = 'ana_datamc_%s.root' % m #if m != 20 else 'ana_datamc_zmumu.root'
...
#    fcn = ROOT.TF1('fcn', 'exp([0] + [1]*x)*x**[2]', lo, hi)
    ...
    s_const.SetName("Const")
    s_const.SetX1NDC(0.73)
    s_const.SetY1NDC(0.44)
    s_const.SetY2NDC(0.69)
...
#     s_const.Draw("e")
    ...
    legend.AddEntry(htot_const,"k factor: OLD", "l")	
    legend.AddEntry(htot, "k factor: NEW", "l")
#     legend.Draw()
    ...
    a1 = fcn_const.GetParameter(1)
    b1 = fcn_const.GetParameter(2)
    c1 = fcn_const.GetParameter(2)
    d1 = fcn_const.GetParameter(3)
    k1 = fcn_const.GetParameter(4)
    ea1 = fcn_const.GetParError(0) 
    eb1 = fcn_const.GetParError(1) 
    ec1 = fcn_const.GetParError(2)
    ed1 = fcn_const.GetParError(3) 
    ek1 = fcn_const.GetParError(4)

    a2 = fcn.GetParameter(1)
    b2 = fcn.GetParameter(2)
    c2 = fcn.GetParameter(2)
    d2 = fcn.GetParameter(3)
    k2 = fcn.GetParameter(4)
    ea2 = fcn.GetParError(0) 
    eb2 = fcn.GetParError(1) 
    ec2 = fcn.GetParError(2)
    ed2 = fcn.GetParError(3) 
    ek2 = fcn.GetParError(4)
    
    xax = htot.GetXaxis()
    hrat = ROOT.TH1F('Ratio_%i_%i' % (lo, hi), ';m(#mu^{+}#mu^{-}) (GeV);kConstant / kFunction', htot.GetNbinsX(), xax.GetBinLowEdge(1), xax.GetBinLowEdge(htot.GetNbinsX()+1))
    for i in xrange(1, hrat.GetNbinsX()+1):
        xlo = xax.GetBinLowEdge(i)
        xhi = xax.GetBinLowEdge(i+1)
        if xlo >= lo and xhi <= hi:
#         	print xlo, xhi, fcn_const.Eval(xlo), fcn.Eval(xlo)
        	f1 = fcn_const.Eval(xlo)
        	f2 = fcn.Eval(xlo)
        	e1 = f1*((ea1 + eb1*xlo + ec1*(xlo**2) + ed1*(xlo**3))*(xlo**k1) + k1*xlo**(k1-1)*ek1)
        	e2 = f2*((ea2 + eb2*xlo + ec2*(xlo**2) + ed2*(xlo**3))*(xlo**k2) + k2*xlo**(k2-1)*ek2)
        	err = ((e1/f2)**2 + (((f1*e2)/(f2**2))**2))**0.5
#         	print xlo, xhi, fcn_const.Eval(xlo), fcn.Eval(xlo), e1, e2, err        	
        	hrat.SetBinContent(i, f1/f2)
        	hrat.SetBinError(i, err)
     	
    hrat.SetMinimum(0.)
    hrat.SetMaximum(2.)
    hrat.GetXaxis().SetRangeUser(lo, hi)
    hrat.Draw()
    
    ps.save('Ratio_%i_%i' % (lo, hi), pdf=True)
